...
import certificateConfig from "@/configs/nav-config/modules/certificate-config.json";
import { Tab, TabProvider } from "@/contexts/TabContext";
import { usePermissions } from "@/hooks/usePermissions";
import { usePathname } from "next/navigation";
import React from "react";

const staticTabs: Tab[] = [{ id: "certificate-dashboard", label: "Certificates", path: "/certificates", type: "list" }];
...
export default function CertificateLayout({ children }: { children: React.ReactNode }) {
    ...
    return <TabProvider staticTabs={staticTabs}><DynamicModuleSidebar config={certificateConfig as any} userPermissions={permissions} /><div className="flex flex-col flex-1 h-screen"><div className="sticky top-0 z-30"><Topbar breadcrumbs={crumbs(pathname)} hasSidebar={true} showFiscalYear={false} /></div><main className="flex-1 overflow-auto bg-background text-foreground">{children}</main></div></TabProvider>;
}
