...
import { Play, Download, Search, CheckCircle2, XCircle, MessageSquare } from "lucide-react";
...
export function ResultProcessingPage() {
    ...
    const handleProcess = async () => {
        ...
        try {
            ...
            toast.error(error.response?.data?.message || "Failed to process results", {
                id: "process-results",
            });
        } finally {
    ...
    const handleOpenSmsPreview = () => {
        if (!activeFilters || resultsData.length === 0) {
            toast.error("Load processed results before sending SMS.");
            return;
        }
        ...
        setSmsPayload({
            sourceType: "result",
            message:
                "Dear {{studentName}}, {{examName}} result: GPA {{gpa}}, Grade {{grade}}, Status {{status}}, Position {{position}}. - {{institutionName}}",
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-sm text-muted-foreground mt-1">
                        Calculate and publish student performance results.
                    </p>
                </div>
                ...
                <Card className="shadow-sm border-primary/10">
                    <CardHeader>
                        ...
                        <CardDescription className="text-xs">
                            Class standings and compiled marks averages.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
                        {isLoading ? (
                            <div className="py-12 text-center text-sm text-muted-foreground">
                                Fetching results registry...
                            </div>
                        ) : resultsData.length === 0 ? (
                            <div className="py-12 text-center text-sm text-muted-foreground">
                                No processed results found. Click "Process" to compile scores.
                            </div>
                        ) : (
                                ...
                                <table className="w-full text-sm">
                                    <thead>
                                        <tr className="border-b text-left">
                                            <th className="pb-3 font-semibold text-muted-foreground text-xs uppercase tracking-wider">
                                                Rank
                                            </th>
                                            <th className="pb-3 font-semibold text-muted-foreground text-xs uppercase tracking-wider">
                                                Roll
                                    ...
                                    <tbody>
                                        {resultsData.map((res: any) => (
                                            ...
                                            >
                                                <td className="py-3">
                                                    <span
                                                        className={`inline-flex items-center justify-center w-6 h-6 rounded-full text-[10px] font-bold ${res.position <= 3 ? "bg-primary text-white" : "bg-muted text-muted-foreground"}`}
                                                    >
                                                        {res.position}
                                                    </span>
                                                </td>
            ...
            {!activeFilters && !isProcessing && (
                <div className="flex flex-col items-center justify-center py-20 border-2 border-dashed rounded-3xl bg-muted/30">
                    ...
                    <p className="text-sm text-muted-foreground mt-2 max-w-sm text-center">
                        Select the session, class, and section parameters above to begin the result
                        calculation process for students.
                    </p>
                </div>
            )}
            {smsPayload && (
